...
function displayCaptures(filenames) {
    if (!filenames || !filenames.length) {
        console.error('No filenames passed to `displayCaptures`', filenames);
        show('uh-oh');
        return;
    }
...
function _displayCapture(filenames, index) {
    ...
    var url = ('chrome-extension://' + chrome.i18n.getMessage('@@extension_id') +
               '/capture/index.html');

    var shortFilename = filename.substring(FSAPI.imgPathBase.length);
    url += '?src=' + encodeURIComponent(shortFilename);
...
function errorHandler(reason) {
    console.error('`errorHandler`', reason);
